...
GITHUB_REPO="AmpleReVanced/revanced-patches"
...
fetch_mpp_from_github() {
    echo ""
    echo -e "${YELLOW}==================================${NC}"
    echo -e "${GREEN}MPP 파일 선택 (Latest + Pre-release)${NC}"
    echo -e "${YELLOW}==================================${NC}"
    echo ""

    echo -e "${BLUE}[INFO] GitHub 릴리스 정보 가져오는 중 (Latest + Pre-release)...${NC}"

    # Latest stable 릴리스 가져오기
    local LATEST_JSON
    LATEST_JSON=$(curl -s "$GITHUB_API_URL/latest" 2>/dev/null)

    # 전체 릴리스 목록 가져오기 (pre-release 포함, 최근 20개)
    local ALL_JSON
    ALL_JSON=$(curl -s "$GITHUB_API_URL?per_page=20" 2>/dev/null)

    if [ -z "$ALL_JSON" ] || echo "$ALL_JSON" | grep -q '"message"'; then
        echo -e "${RED}[ERROR] GitHub API 요청 실패. 인터넷 연결을 확인하세요.${NC}"
        return 1
    fi

    # Latest + Pre-release 합치기 (tag_name 기준 중복 제거)
    local RELEASES_JSON
    if command -v jq &> /dev/null && echo "$LATEST_JSON" | jq -e '.tag_name' &>/dev/null 2>&1; then
        RELEASES_JSON=$(jq -n \
            --argjson latest "[$LATEST_JSON]" \
            --argjson all "$ALL_JSON" \
            '$latest + $all | unique_by(.tag_name)')
        [ -z "$RELEASES_JSON" ] && RELEASES_JSON="$ALL_JSON"
    else
        RELEASES_JSON="$ALL_JSON"
    fi
    ...
    if command -v jq &> /dev/null; then
        ...
        while IFS= read -r line; do
            MPP_URLS+=("$line")
        done < <(echo "$RELEASES_JSON" | jq -r '.[] | .assets[] | select(.name | endswith(".mpp") and (contains("sources") | not) and (contains("javadoc") | not)) | .browser_download_url' | head -20)

        # 이름에 [Latest: v...] 또는 [Pre-release: v...] 라벨 추가
        while IFS= read -r line; do
            MPP_NAMES+=("$line")
        done < <(echo "$RELEASES_JSON" | jq -r '.[] | . as $rel | .assets[] | select(.name | endswith(".mpp") and (contains("sources") | not) and (contains("javadoc") | not)) | "\(.name)  [\(if $rel.prerelease then "Pre-release" else "Latest" end): \($rel.tag_name)]"' | head -20)
    else
        # jq 없을 때 기본 파싱
        while IFS= read -r line; do
            RELEASE_TAGS+=("$line")
        done < <(echo "$RELEASES_JSON" | grep -oP '"tag_name"\s*:\s*"\K[^"]+' | head -20)
    ...
    echo ""
    echo -e "${GREEN}사용 가능한 MPP 버전 (Latest + Pre-release):${NC}"
